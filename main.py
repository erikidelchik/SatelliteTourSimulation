...
import random
import sys
# ── run ONCE in the same folder as your orbit script ──
from PIL import Image, ImageDraw     # pip install pillow

size   = 1024                        # square texture, power-of-two is safe
radius = 0.35                        # hole = 35 % of width
...
def generate_evenly_spaced_targets(track_latlon,
    ...
    # 1) Build a list of cumulative distances along track_latlon
    ...
    # 2) Compute how far apart each target should be
    ...
    # 3) For each of the n_points, find the appropriate index
    for _ in range(n_points):
        ...
        if lateral_km > 0:
            # 4a) If lateral_km > 0, randomly offset that base point by up to lateral_km
            bearing_rand = random.uniform(0, 360)
            tgt_lat, tgt_lon = gc_destination(base_lat, base_lon, bearing_rand, lateral_km)
        else:
            # 4b) No offset: target is exactly on the ground track
...
# we generate list of the targets
targets = generate_evenly_spaced_targets(track_samples,
                                         n_points=20,
                                         lateral_km=0)

targets_named = [(lat, lon, f"Target #{i + 1}") for i, (lat, lon) in enumerate(targets)]

# Creating the targets (new points)
...
# ──────────────────────────────────────────────────────────────
# A  Give each target a stable id so AnimatedUpdate can address it
# ──────────────────────────────────────────────────────────────
target_ids = {}
for lat, lon, name in targets_named:
    pm = kml.newpoint(name=name, coords=[(lon, lat)])
...
for i in range(0, len(t_span) - idx_step, idx_step):
    t_now = t_span[i]
    θ_now = ang_velocity * t_now # 0-Current lap angle in radians
    sat_lat, sat_lon = eci_to_llh(θ_now) # The geographical position of the satellite on the Earth's surface
    ...
    for tgt_lat, tgt_lon, tgt_name in targets_named:
        if gc_distance(sat_lat, sat_lon, tgt_lat, tgt_lon) <= range_km:
            watch_target = (tgt_lat, tgt_lon, tgt_name)
            break

    # Build the next FlyTo
    fly = playlist.newgxflyto(gxduration=step_cam) # key-frame are the time points where the camera state is explicitly defined
    ...
    if watch_target is not None:
        ...
        fly.camera.tilt = 15  # a normal state where the camera looks down
